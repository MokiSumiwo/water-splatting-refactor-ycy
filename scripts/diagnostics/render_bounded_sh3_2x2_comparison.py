#!/usr/bin/env python
"""Render Bounded-SH3 four-path step-15000 comparison sheets."""
...
def _assert_matching_views(summaries: Mapping[str, Mapping[str, Any]]) -> Tuple[List[int], List[int], List[str]]:
    first = next(iter(summaries.values()))
    view_ids = [int(row["view_id"]) for row in first.get("per_view", [])]
    camera_ids = [int(row["camera_id"]) for row in first.get("per_view", [])]
    image_names = [str(row.get("image_name", "")) for row in first.get("per_view", [])]
    for name, summary in summaries.items():
        current_views = [int(row["view_id"]) for row in summary.get("per_view", [])]
        current_cameras = [int(row["camera_id"]) for row in summary.get("per_view", [])]
...
def render(args: argparse.Namespace) -> Dict[str, Any]:
        ...
        manifest.append(
            {
                ...
                "width": width,
                "height": height,
                "display_logic": "reuse diagnostic PNG mapping; no auto exposure, WB, gamma, histogram equalization, or per-image normalization",
            }
        )

    components = [
        ("underwater", "contact_sheet_underwater_2x2.png", "underwater_rgb", True),
        ("direct_object_signal", "contact_sheet_direct_object_signal_2x2.png", "direct_object_signal", False),
        ("clear_raw", "contact_sheet_clear_raw_2x2.png", "clear_object_fullsh_raw_display", False),
        ("clear_clamp01", "contact_sheet_clear_clamp01_2x2.png", "clear_object_fullsh_clamp01", False),
        ("clear_ws_tonemap", "contact_sheet_clear_ws_tonemap_2x2.png", "clear_object_fullsh_ws_tonemap", False),
        ("transmission", "contact_sheet_transmission_2x2.png", "transmission", False),
        ("tau_D", "contact_sheet_tau_d_2x2.png", "tau_D_visualization", False),
    ]
    ...
    lines = [
        "# Bounded SH3 Step-15000 Four-Path Visual Comparison",
        "",
        f"Scene: {args.scene}",
...
def main() -> None:
    ...
    parser.add_argument("--run", action="append", required=True, help="Run root as NAME=diagnostic/root")
