#!/usr/bin/env python3
"""Read-only Curasao pure-water/background-anchor observability audit.

This diagnostic reuses the locked Panama BND-PW-AUDIT mask semantics on
Curasao. It loads existing checkpoints, renders fixed train/eval cameras,
...
SCENE = "Curasao"
OUTPUT_DIR = Path("outputs/bnd_pw_audit_curasao_20260817")
RESEARCH_NOTE = Path("research_notes/BND_PW_AUDIT_CURASAO_2026-08-17.md")
DATA_PATH = Path("undistorted_data/undistorted_Curasao")
...
def _stats_np(values: np.ndarray, prefix: str) -> Dict[str, Any]:
    ...
    if vals.size == 0:
        return {f"{prefix}{key}": float("nan") for key in ("mean", "p50", "p90", "p99", "min", "max")}
    return {
...
def _summarize_classification(
    ...
    if coverage_ok and contamination_ok and low_object and headroom_ok and grad_ok and stable_views:
        classification = "BG_ANCHOR_READY"
        next_step = "BND + CB-BG-ONLY on Curasao"
    elif (safe_coverage > 0.0 and (headroom_ok or grad_ok)) or views_ge1 > 0:
        classification = "BG_ANCHOR_WEAK"
        next_step = "Apply the same locked BND-PW-AUDIT to IUI3; do not train CB-BG on Curasao"
    else:
        classification = "BG_ANCHOR_NOT_SUPPORTED"
        next_step = "Apply the same locked BND-PW-AUDIT to IUI3 only if continuing the BG-anchor screen; do not train CB-BG on Curasao"
    return {
...
def _write_research_note(path: Path, summary: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"INFERENCE: Q8 final classification is `{cls['classification']}`.",
        f"RECOMMENDATION: Q9 one next experiment is `{cls['next_single_experiment']}`.",
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf8")
...
def run(args: argparse.Namespace) -> None:
    ...
    camera_manifest = {
        ...
        "mask_thresholds_locked_before_this_curasao_run": True,
    ...
    classification = _summarize_classification(coverage_rows, headroom_rows, contamination_rows, gradient_summary, view_rows)
    summary = {
        "repo": repo_manifest,
