...
DEPTH_GRID="${DEPTH_GRID:-D0:0.75:0.10 D1:0.80:0.10 D2:0.75:0.15 D3:0.80:0.15}"
STAMP="${STAMP:-$(date -u +%Y%m%d_%H%M%S)}"
export STAMP

for ITEM in ${DEPTH_GRID}; do
  TAG="${ITEM%%:*}"
  REST="${ITEM#*:}"
  export INFINITE_WATER_DEPTH_MID="${REST%%:*}"
  export INFINITE_WATER_DEPTH_TEMP="${REST#*:}"
  export EXPERIMENT_NAME="${EXPERIMENT_NAME_PREFIX:-m2_p2_depth}_${TAG}_mid${INFINITE_WATER_DEPTH_MID/./p}_temp${INFINITE_WATER_DEPTH_TEMP/./p}_accum${ACCUM_ZERO_WEIGHT/./p}_seed${SEED}_${MEDIUM_CONTEXT_MODE}_iui3_redsea_${MAX_NUM_ITERATIONS}"
  unset TIMESTAMP
