...
GPU="${GPU:-0}"
SEED="${SEED:-42}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_DIR}/outputs/bnd_hr_panama_20260810}"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-panama_bnd_hr_seed${SEED}_step0_to_${MAX_NUM_ITERATIONS}}"
TIMESTAMP="${TIMESTAMP:-20260810_bnd_hr}"
...
CUDA_VISIBLE_DEVICES="${GPU}" "${NS_TRAIN}" water-splatting \
  ...
  --pipeline.model.intrinsic-color-parameterization bounded_headroom_sh3 \
